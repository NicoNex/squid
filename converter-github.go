...
func NewConverter() *Converter {
    return &Converter {
        ...
        opt: &github.MarkdownOptions{Mode: "markdown"},
        ctx: context.Background(),
    }
}

func (c *Converter) Convert(in []byte) string {
    html, _, err := c.client.Markdown(c.ctx, string(in), c.opt)
    check(err)
    return html
}

func (c Converter) AddStyle(in string) string {
    var style = `<style>
